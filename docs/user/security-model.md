...
| **`NET_ADMIN` blast radius** | The app runs as root in the container holding exactly one capability, `NET_ADMIN`, in the host network namespace. A bug that let an attacker run code in it could flush or fill the set and change host routes, interfaces or firewall rules. It could not read host files, load kernel modules or use any other capability. `cap_drop: ALL`, `no-new-privileges` and a read-only filesystem keep this as small as it can be. |
